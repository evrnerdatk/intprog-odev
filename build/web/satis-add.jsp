    ...
    <div class="row">
      <div class="col-sm-9 col-md-7 col-lg-5 mx-auto">
        <div class="card card-signin my-5">
            <div class="card-body">
				<c:if test="${satis != null}"> <!satıs null degerine eşit mi değil mi kontrolü yapıyoruz yani satisi nulla eşit değilse güncelleme işlemi yapıyoruz>
					<form action="SatisServlet/update" method="post">
				</c:if>
				<c:if test="${satis == null}">
					<form action="SatisServlet/insert" method="post">
				</c:if>
				...
				<c:if test="${satis != null}">
					<input type="hidden" name="id" value="<c:out value='${satis.id}' />" />
				</c:if>
                                        <fieldset class="form-group">
                                            <label><strong>Urun</strong></label>
                                            <br>
				<select name="urun" id="urun" class="form-control"> 
                                    <!foreach döngüsü ile listurun değişkenini çağırıyoruz>
                                    <c:forEach var="urun" items="${listUrun}">
                                        <option value="${urun.id}">${urun.kategori}-${urun.renk}-${urun.marka}-Stok Adedi=${urun.stokAdedi}</option>
                                    </c:forEach>
					</select>
				...
				<fieldset class="form-group">
					...
					<select name="musteri" id="musteri" class="form-control">
						...
						<option value="${musteri.id}">${musteri.fName} ${musteri.lastName}</option>
                                            </c:forEach>
					</select>
                                  </fieldset>
                                        <br>
                                        ...
                                        <input type="text" id="stokAdedi" value="<c:out value='${satis.stokAdedi}' />" class="form-control" name="stokAdedi" required="required">
                                        <label>Adet</label>
                                </div>
                                        <br>
