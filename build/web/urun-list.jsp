...
<div class="container">
	<div class="row">
		...
		<div class="container">
                    <br>
			<h3 class="text-center">Urun Listesi</h3>
			<hr>
			<br>
			<table class="table">
				...
				<tbody>
					...
					<c:forEach var="urun" items="${listUrun}">
						...
						<tr>
							<td style="color: white"><c:out value="${urun.id}" /></td>
							<td style="color: white"><c:out value="${urun.kategori}" /></td>
							<td style="color: white"><c:out value="${urun.marka}" /></td>
							<td style="color: white"><c:out value="${urun.renk}" /></td>
							<td style="color: white"><c:out value="${urun.stokAdedi}" /></td>
                                                        <c:if test="${loggedUser.email == 'admin' && loggedUser.password == 'admin'}"> 
							<td> <a style="color:red" href="<%=request.getContextPath()%>/UrunServlet/delete?id=<c:out value='${urun.id}' />">Sil</a>
                                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                                            <a href="<%=request.getContextPath()%>/UrunServlet/edit?id=<c:out value='${urun.id}' />">Düzenle</a>
				...
				<a href="<%=request.getContextPath()%>/UrunServlet/new" class="btn btn-danger">Urun Ekle</a>
			</div>
                                </c:if>
